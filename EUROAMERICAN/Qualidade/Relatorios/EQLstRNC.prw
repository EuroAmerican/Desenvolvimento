...
TRCell():New(oSection1,"UI_OCDESC"	,		,"Desc Ocorr."	,	,	,.F.	,,,,,,,.F.)  
TRCell():New(oSection1,"UI_CAUTEC"	,		,"Cod. C. Tec." ,	,	,.F.	,,,,,,,.F.)  
TRCell():New(oSection1,"UI_CAUDSC"	,		,"Desc C. Tec."	,	,	,.F.	,,,,,,,.F.)  
TRCell():New(oSection1,"UI_VEND"	,		,"Cod. Vendedor",	,	,.F.	,,,,,,,.F.)  
TRCell():New(oSection1,"UI_DSCVEN"	,		,"Nome Vendedor",	,	,.F.	,,,,,,,.F.)  
TRCell():New(oSection1,"UI_TRANSP"	,		,"Cod. Transp.",	,	,.F.	,,,,,,,.F.)  
TRCell():New(oSection1,"UI_DSCTRAN"	,		,"Nome Transp.",	,	,.F.	,,,,,,,.F.)  
...
//Local aDados[16]
Local aDados[28] // Alterado por Fabio CARNEIRO 03/05/2022
...
// Alterado por Fabio Carneiro 28/04/2022

oSection1:Cell("UI_MOTDEVO"	):SetBlock( { || aDados[15]})
oSection1:Cell("UI_OCDESC"	):SetBlock( { || aDados[16]})
oSection1:Cell("UI_CAUTEC"	):SetBlock( { || aDados[17]})
oSection1:Cell("UI_CAUDSC"	):SetBlock( { || aDados[18]})
oSection1:Cell("UI_VEND"	):SetBlock( { || aDados[19]})
oSection1:Cell("UI_DSCVEN"	):SetBlock( { || aDados[20]})
oSection1:Cell("UI_TRANSP"	):SetBlock( { || aDados[21]})
oSection1:Cell("UI_DSCTRAN"	):SetBlock( { || aDados[22]})
oSection1:Cell("UI_NCONFOR"	):SetBlock( { || aDados[23]})

//Alterado por Fabio Batista 26/11/2020
oSection1:Cell("UI_ANALISE"	):SetBlock( { || aDados[24]})
oSection1:Cell("UI_CAUSA"	):SetBlock( { || aDados[25]})
oSection1:Cell("UI_PLANO"	):SetBlock( { || aDados[26]})
oSection1:Cell("UI_DETIMPL"	):SetBlock( { || aDados[27]})
oSection1:Cell("UI_DETEFIC"	):SetBlock( { || aDados[28]})
...
cQuery	:= "SELECT UI_CODIGO, UI_EMISSAO, UI_CODCLI, UI_LOJA, UI_NOMECLI, UI_SNOTA, UI_OCDESC, UI_CAUTEC,UI_CAUDSC, UI_VEND,UI_DSCVEN,UI_TRANSP,UI_DSCTRAN, "+CRLF
...
While !(cAlias)->(Eof()) 
	...
	// Alterado Fabio carneiro 29/04/2022

	aDados[15]	:= (cAlias)->UI_MOTDEVO
	aDados[16]	:= (cAlias)->UI_OCDESC   // AllTrim(Posicione("ZZK",1,xFilial("ZZK")+(cAlias)->UI_MOTDEVO,"ZZK_DESC")) - Paulo Lenzi 26.07.2023
	aDados[17]	:= (cAlias)->UI_CAUTEC
	aDados[18]	:= AllTrim(Posicione("ZZJ",1,xFilial("ZZJ")+(cAlias)->UI_CAUTEC,"ZZJ_DESC"))
	aDados[19]	:= (cAlias)->UI_VEND
	aDados[20]	:= AllTrim(Posicione("SA3",1,xFilial("SA3")+(cAlias)->UI_VEND,"A3_NOME"))
	aDados[21]	:= (cAlias)->UI_TRANSP
	aDados[22]	:= AllTrim(Posicione("SA4",1,xFilial("SA4")+(cAlias)->UI_TRANSP,"A4_NOME"))
	aDados[23]	:= (cAlias)->UI_NCONFOR
	
	//Alterado Fabio Batista 26/11/2020
	aDados[24]	:= (cAlias)->UI_ANALISE
	aDados[25]	:= (cAlias)->UI_CAUSA
	aDados[26]	:= (cAlias)->UI_PLANO
	aDados[27]	:= (cAlias)->UI_DETIMPL
	aDados[28]	:= (cAlias)->UI_DETEFIC
