...
/*
+------------+-----------+--------+---------------+-------+--------------+
| Programa:  | ImpSB1Bd  | Autor: |Fabio Carneiro | Data: | 10/11/21     |
+------------+-----------+--------+---------------+-------+--------------+
| Descrição: | Atualiza preço de venda - solicitação Eristeu             |
+------------+-----------------------------------------------------------+
| Uso:       | QUALY                                                     |
+------------+-----------------------------------------------------------+
*/
User Function QEIMPPRC() 
...
Private mv_par01    := space(30) // diretório + arquivo a ser migrado
...
Define MsDialog oDlg1 From 000,000  TO 015,060 Title OemToAnsi("Atualiza Preço de Venda no Cadastro do Produto - QUALY") 
@ 003,005 TO 090,230
...
/*
+------------+-----------+--------+---------------+-------+--------------+
| Programa:  | OkLeTxt   | Autor: |Fabio Carneiro | Data: | 10/11/21     |
+------------+-----------+--------+---------------+-------+--------------+
...
_B1_PRV1   := Val(StrTran(StrTran(Alinha[1][02],";",""),",", "."))

If Select("WK_SB1") > 0
	WK_SB1->(DbCloseArea())
EndIf

_cQuery := "SELECT B1_FILIAL, B1_COD, B1_PRV1 FROM "+RetSqlName("SB1")+" SB1 "
...
Else 
	...
	aAdd(aPlanilha,{"REGISTRO NÃO ALTERADO",;
							AllTrim(_B1_PRODUTO),;
									_B1_PRV1})
...
Static Function QESB101ok()
	...
	Local cArqDst     := "C:\TOTVS\QESB1PRC_EMP_" + SM0->M0_CODIGO + "_" + Dtos(dDataBase) + StrTran(Time(),":","") + ".XML"
	...
	oExcel:AddColumn(cNomPla, cTitPla, "Preço de Venda"        , 3, 2, .F.)     //03
