...
#DEFINE ROTINA_FILE				"QECTB001.prw"
...
#Define ALIAS_FORM0 			"ZZC" //Informe o Alias da Tabela
#Define MODELO					"QECTB001"
#Define ID_MODEL				"QECTBM01"
#Define TITULO_MODEL			"Cadastro Operação Contabil " + SubStr(VERSAO_ROTINA,1,17)
...
/*/{Protheus.doc} QECTB001 - cadastro de operações contabeis 
Fonte Modelo de MVC - Interface Modelo 1 para cadastro de operações  
@type function
...
/*/ 
User Function QECTB001(aAutoCab,nOperacao)
	...
	Else	
		...
		/*
        Quando os tipos forem 
		1=Fiscal
		2=Financeiro
		3=Ativo Imobnilizado
		4=Outros
        */
		oFwMBrowse:AddLegend( cAliasForm+"_TIPO == '1'  ", "BR_VERDE   " , "Fiscal" )
		oFwMBrowse:AddLegend( cAliasForm+"_TIPO == '2'  ", "BR_AZUL    " , "Financeiro" )
		oFwMBrowse:AddLegend( cAliasForm+"_TIPO == '3'  ", "BR_PRETO   " , "Ativo Imob." )
		oFwMBrowse:AddLegend( cAliasForm+"_TIPO == '4'  ", "BR_VERMELHO" , "Outros" )
...
Static Function exportModeltoExcel()
	...
	FwMsgRun( ,bAcao, "EURO", "Exportando Dados para o Excel..." )
...
Static Function posValid(oModel)
	...
	Local cCpoFilial	:= cPrefForm+"_FILIAL"
	Local cCpoCodigo	:= cPrefForm+"_COD"
	Local cFilCodigo	:= xFilial(cAliasForm) 
	Local cCodigo		:= oModel:GetValue(cIDModelForm,cCpoCodigo)	
	...
	If nOperation = MODEL_OPERATION_INSERT
		...
		Begin Sequence
			...
			If (cAliasForm)->(dbSeek(cFilCodigo+cCodigo))
				Help(,,"HELP",,"Atenção! A regra informada '" + AllTrim(cCodigo) + "' não pode ser utilizada pois já existe na base de dados para esta filial.",1,0)	
				Break
			Endif
