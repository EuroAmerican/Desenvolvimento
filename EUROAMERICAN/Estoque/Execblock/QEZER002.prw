...
If nOpca == 1
	...
	Else 
		...
		Else 
			Aviso('EQZER002','Rotina não Autorizada a Executar, Abrir Chamado para o uso desta rotina!',{'OK'})
			Return
		EndIf	
	Endif
...
Local cTitPla    := "QEZER002 - Relatorio Conferência do Inventário com quantidade Zero "+Substr(DtoS(MV_PAR09),7,2)+"/"+Substr(DtoS(MV_PAR09),5,2)+"/"+Substr(DtoS(MV_PAR09),1,4)+" "
...
oExcel:AddColumn(cNomPla, cTitPla, "Controla Endereço?" , 1, 1, .F.)     //16
oExcel:AddColumn(cNomPla, cTitPla, "Produto Bloqueado?" , 1, 1, .F.)     //17
oExcel:AddColumn(cNomPla, cTitPla, "Produto Empenhado?" , 1, 1, .F.)     //18
oExcel:AddColumn(cNomPla, cTitPla, "Status Inventário?" , 1, 1, .F.)     //19
...
While TRB1->(!Eof())
	...
	Incproc("Atualizando registro de Inventário: " + TRB1->B8_PRODUTO)
				...
				For _nY := 1 To len(_aPassa) 
					...
					If SB7->(dbSeek(xFilial("SB7")+_aPassa[_nY][01]+_aPassa[_nY][02]+_aPassa[_nY][03]+_aPassa[_nY][04]+_aPassa[_nY][05]+_aPassa[_nY][06]+_aPassa[_nY][07]+_aPassa[_nY][08]))
						...
						aAdd(aPlanilha,{_cStatus,;          //01
								...
								"Quant. Zero",;  //14
								If(TRB1->B1_RASTRO=="L","Sim","Não"),;  //15
								If(TRB1->B1_LOCALIZ=="S","Sim","Não"),; //16
								If(TRB1->B1_MSBLQL=="1","Sim","Não"),;  //17 
								If(TRB1->B8_EMPENHO > 0 ,"Sim","Não"),; //18
								If(_aPassa[_nY][10]=="1","Não Processado","Processado"),;//19
...
Aadd(_aPerg,{"Data Inventário....?","mv_ch9","D",08,"G","mv_par09","","","","","",""   ,"","",0})
