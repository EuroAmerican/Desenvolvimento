...
/*/{Protheus.doc} MTA200MNU
Ponto de entrada para fazer o calculo do Custo Net e Custo Test 
...
User Function MTA200MNU()      
		...
		If cFilant $ "0803/0901/0200"
			AADD(aRotina, {"Atualiza Processos Net","U_QEUPDPAZ",0,6}) 
		Else 
			Aviso("A T E N Ç Ã O - MTA200MNU", "A L E R T A  -  Esta Rotina somente pode ser utilizada na Qualy ou Phoenix !!! ", {"Ok"})
			Return()
		EndIf

Return()
/*/{Protheus.doc} QEUPDPAZ
...
User Function QEUPDPAZ()

Local aSays    := {}
Local aButtons := {}
Local cTitoDlg := "Cáculo do custo Net e Custo Test - Estrtura de Produtos"
Local nOpca    := 0
Private _cPerg := "QEUPDCT"
...
aAdd(aSays, "Esta Rotina Somente deve Ser utilizada na Qualy ou Phoenix !")
...
cQuery1 += "INNER JOIN "+RetSqlName("SG1")+" AS SG1 ON G1_FILIAL IN "+FORMATIN(AllTrim(MV_PAR02),"/")+" " + CRLF
...
cQuery1 += " AND B1_COD = '"+MV_PAR01+"' " + CRLF
...
cQryA += " AND PAZ_COD = '"+MV_PAR01+"' " + CRLF
...
If cFilant $ "0803" .Or. cFilant $ "0901"

	If _lGravou .And. !Empty(Posicione("SB1",1,xFilial("SB1")+MV_PAR01,"B1_COD"))

		_cTexto += '<b><font size="3" face="Arial">O Produto abaixo foi Atualizado Custo Net/Test pelo Usuário: '+AllTrim(cNomeUsr)+'</font></b><br><br>'
		_cTexto += '<b><font size="2" face="Arial">Código: '+Posicione("SB1",1,xFilial("SB1")+MV_PAR01,"B1_COD")+'</b><br>'
		_cTexto += '<b><font size="2" face="Arial">Descrição: '+Posicione("SB1",1,xFilial("SB1")+MV_PAR01,"B1_DESC")+'</b><br>'
		If cFilant $ "0803"
			_cTexto += '<b><font size="2" face="Arial">Custo Net R$:  '+Transform(Posicione("SB1",1,xFilial("SB1")+MV_PAR01,"B1_CNETQUA") , "@R 999,999,999.9999")+'</b><br>'
			_cTexto += '<b><font size="2" face="Arial">Custo Test R$: '+Transform(Posicione("SB1",1,xFilial("SB1")+MV_PAR01,"B1_VALOR4Q"), "@R 999,999,999.9999")+'</b><br>'
		EndIf
		If cFilant $ "0901"	
			_cTexto += '<b><font size="2" face="Arial">Custo Net R$: '+Transform(Posicione("SB1",1,xFilial("SB1")+MV_PAR01,"B1_CNETPHO") , "@R 999,999,999.9999")+'</b><br>'
			_cTexto += '<b><font size="2" face="Arial">Custo Test R$:'+Transform(Posicione("SB1",1,xFilial("SB1")+MV_PAR01,"B1_VALOR4P"), "@R 999,999,999.9999")+'</b><br>'
		EndIf
		_cTexto += '<b><font size="2" face="Arial" Color="Red">Observação: Este produto deve ser realizado a confêrencia do Custo Net na Empresa/Filial '+AllTrim(SM0->M0_FILIAL)+'</font></b><br>'
		_cTexto += '<br><br><b><font size="3" face="Arial" color="Blue">'+AllTrim(SM0->M0_NOME)+' - '+AllTrim(SM0->M0_FILIAL)+'</font></b><br>'
		_cTexto += '</html>'

		_cDest := GetMV("QE_MAILSG1")
			
		If !Empty(_cDest).And.Len(_cTexto)>30
			U_CPEmail(_cDest,"","Atualização de Custo Net/Test do Produto : "+Posicione("SB1",1,xFilial("SB1")+MV_PAR01,"B1_COD"),_cTexto,"",.T.)
		EndIf
...
Aadd(_aPerg,{"Codigo Produto ....?","mv_ch1","C",15,"G","mv_par01","","","","","","SB1","","",0})
Aadd(_aPerg,{"Qual Filial .......?","mv_ch2","C",20,"G","mv_par02","","","","","","SM0","","",0})
Aadd(_aPerg,{"Tipo de Custo   ...?","mv_ch3","C",01,"C","mv_par03","Test","Stardart","","","","","","",0})
