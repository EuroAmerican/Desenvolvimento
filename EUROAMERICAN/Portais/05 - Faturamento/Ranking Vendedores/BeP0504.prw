...
&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	...
	If lDiretor
		...
		cQuery += "SELECT '07' AS FILIAL, 'Jays' AS EMPRESA, CASE WHEN D2_SEGUM = 'L' AND B1_CONV <> 0 THEN D2_QTSEGUM ELSE D2_QUANT END AS VOLUME, CASE WHEN B1_SEGUM = 'L' AND B1_CONV <> 0 THEN CASE WHEN B1_TIPCONV = 'M' THEN D2_PRCVEN / B1_CONV ELSE D2_PRCVEN * B1_CONV END ELSE D2_PRCVEN END AS PRECO " + CRLF
		cQuery += "FROM " + RetSqlName("SD2") + " AS SD2 WITH (NOLOCK) " + CRLF
		cQuery += "INNER JOIN " + RetSqlName("SF2") + " AS SF2 WITH (NOLOCK) ON F2_FILIAL = D2_FILIAL " + CRLF
		cQuery += "  AND F2_DOC = D2_DOC " + CRLF
		cQuery += "  AND F2_SERIE = D2_SERIE " + CRLF
		...
		Do While !VOLMED->( Eof() )
			...
			ElseIf AllTrim( VOLMED->EMPRESA ) == "Jays"
...
&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	...
	If (lFilUsr .And. AllTrim( HttpSession->ccodusr ) $ cSoQualy) .Or. !lFilUsr
		...
		cHtml += '    <div class="form-group col-md-12">'+CRLF
		cHtml += '        <H3><i class="fa fa-bars fa-1x"></i> Posição do Faturamento por Vendedores - JAYS</H3>'+CRLF
		cHtml += '    </div>'+CRLF
