...
/*/
//--------------------------------------------------------------------
User Function UPDA1LIB( cEmpAmb, cFilAmb )

Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
Local   cTitulo   := "ATUALIZAÇÃO DE DICIONÁRIOS E TABELAS"
Local   cDesc1    := "Esta rotina tem como função fazer  a atualização  dos dicionários do Sistema ( SA1 )"
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização Realizada.", "UPDA1LIB" )
				Else
					MsgStop( "Atualização não Realizada.", "UPDA1LIB" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX3, { ;
	...
	{ 'Lib. Cad.'															, .T. }, ; //X3_TITSPA
	{ 'Lib. Cad.'															, .T. }, ; //X3_TITENG
	{ 'Liberação Cadastral'													, .T. }, ; //X3_DESCRIC
	{ 'Liberação Cadastral'													, .T. }, ; //X3_DESCSPA
	{ 'Liberação Cadastral'													, .T. }, ; //X3_DESCENG
	{ '@!'																	, .T. }, ; //X3_PICTURE
	{ ''																	, .T. }, ; //X3_VALID
...
//
aHlpPor := {}
aAdd( aHlpPor, 'Liberação Cadastral' )

aHlpEng := {}
aAdd( aHlpEng, 'Liberação Cadastral' )

aHlpSpa := {}
aAdd( aHlpSpa, 'Liberação Cadastral' )

PutSX1Help( "PA1_XLIBCAD", aHlpPor, aHlpEng, aHlpSpa, .T.,,.T. )
AutoGrLog( "Atualizado o Help do campo " + "A1_XLIBCAD" )
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPDA1LIB" ) ) ) ;
