...
User Function QEACD001()

    Local aSays    := {}
	Local aButtons := {}
	Local nOpca    := 0
	Local cTitoDlg := "GRAVA STATUS COLETOR DE DADOS SEPARAÇÃO MANUAL"
    Private _cPerg := "QEACDRA1"

	// Prepara e executa a pergunta do relatorio

	aAdd(aSays, "Esta rotina tem por objetivo gravar status da ordem de separação!")
    ...
    aAdd(aSays, "Para voltar o ststus da OS, Somente Após a nota fiscal cancelada !!!! ")
...
cQuery += " AND CB7_ORDSEP BETWEEN '"+MV_PAR01+"' AND '"+MV_PAR02+"'  " + ENTER
cQuery += " AND NOT CB7_STATUS = '9' " + ENTER  
...
While TRB1->(!Eof())
	...
	If lPassa
		...
		cQuerya := "UPDATE " + RetSqlName("CB7") + " SET CB7_STATUS = '9', " + ENTER
		cQuerya += " CB7_CODOPE = '"+AllTrim(MV_PAR03)+"', " + ENTER
		cQuerya += " CB7_DTEMIS = '"+Dtos(dDataBase)+"',     " + ENTER
		cQuerya += " CB7_HREMIS = '"+SubStr(Time(),1,2)+":"+SubStr(Time(),4,2)+"',  " + ENTER
		cQuerya += " CB7_DTFIMS = '"+Dtos(dDataBase)+"',     " + ENTER
		cQuerya += " CB7_HRFIMS = '"+SubStr(Time(),1,2)+":"+SubStr(Time(),4,2)+"',  " + ENTER
		cQuerya += " CB7_TIPEXP = '00*08*10*',  " + ENTER
		cQuerya += " CB7_PEDIDO = '"+_cPedvendas+"',  " + ENTER
		cQuerya += " CB7_LOCAL  = '08'  " + ENTER
		...
		cQuerya += " AND NOT CB7_STATUS = '9'  " + ENTER  
...
If lPassa
	...
	MsgInfo("O.S Finalizada Com Sucesso!!!")
...
Aadd(_aPerg,{"Ordem Sep. De  ..?"        ,"mv_ch1","C",06,"G","mv_par01","","","","","","CB7","","",0})
Aadd(_aPerg,{"Ordem Sep. Até ..?"        ,"mv_ch2","C",06,"G","mv_par02","","","","","","CB7","","",0})
Aadd(_aPerg,{"Operador   ......?"        ,"mv_ch3","C",06,"G","mv_par03","","","","","","CB1ACD","","",0})
